...
<header>
  ...
  <p>支持任意A股标的动态获取 · 可调快线/慢线天数 · 自动计算累计回报 / 夏普比率 / 最大回撤（MDD）｜ 默认数据：Tushare前复权 · 动态获取：腾讯财经前复权 · 涨红跌绿</p>
</header>
...
<footer>均线交叉策略回测 · 仅供教学研究，不构成投资建议 · 默认数据：Tushare · 动态获取：腾讯财经</footer>
</div>
...
function run(){
  ...
  const src=EMBEDDED.has(code)?'Tushare':'腾讯财经';
  let warn='';
...
function marketTag(code){
  ...
  const c=code[0];
  if(c==='6')return'sh'+code;
  if(c==='8'||c==='4')return'bj'+code;
  return'sz'+code;
}
...
function fetchStock(inputCode){
  ...
  const tag=marketTag(code);
  const url=`https://web.ifzq.gtimg.cn/appstock/app/fqkline/get?param=${tag},day,2024-01-01,,500,qfq`;

  fetch(url).then(r=>r.json()).then(data=>{
    btn.innerHTML=orig;btn.disabled=false;
    if(!data||data.code!==0||!data.data||!data.data[tag]||!data.data[tag].qfqday){
      toast('未获取到 '+code+' 的数据，请检查代码是否正确','err');return;
    }
    const info=data.data[tag];
    const klines=info.qfqday;
    const name=(info.qt&&info.qt[1])?info.qt[1]:code;
    if(klines.length<30){toast(code+' 数据不足（仅'+klines.length+'条），无法回测','err');return;}
    const dates=[],open=[],high=[],low=[],close=[],volume=[];
    for(const kl of klines){
      // Format: [date, open, close, high, low, volume]
      dates.push(kl[0]);open.push(+kl[1]);close.push(+kl[2]);
      high.push(+kl[3]);low.push(+kl[4]);volume.push(+kl[5]);
    }
